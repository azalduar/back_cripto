...
@section('main-content')
    ...
    <div class="row">
        <div class="col-md-12">
                        ...
                        <table id="dataTable" class="table table-hover table-condensed">
                            <thead>
                                <tr>
                                    <th width="3%">ID</th>
                                    <th>Name</th>
                                    @if(Auth::user()->can('categories_update'))
            ...
            @if(Auth::user()->can('categories_create'))
            <a class="btn btn-primary" href="{{url('/categories/create')}}" role="button">{{trans('message.create')}}</a>
            @endif
        </div>
    </div>        
    @if(Auth::user()->can('categories_delete'))
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h5>{{trans('message.warning_delete')}} <b><span id="categoryId"></span></b> ?</h5>
                </div>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('#deleteModal').on('show.bs.modal', function (event) {
                ...
                var modal = $(this);
                modal.find('#deleteForm').attr('action', '{{url('categories')}}/'+category_id);
                modal.find('#categoryId').text(category_id);
            });
            @endif
