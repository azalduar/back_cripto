...
@section('main-content')
    ...
    <div class="row">
        <div class="col-md-12">
                        ...
                        <table id="dataTable" class="table table-hover table-condensed">
                            ...
                            @foreach($tags as $tag)
                                <tr>
                                    ...
                                    @if(Auth::user()->can('tags_delete'))
                                    <td>
                                        <a href="#" data-toggle="modal" data-target="#deleteModal" data-id="{{$tag->id}}">
            ...
            <a class="btn btn-primary" href="{{url('/tags/create')}}" role="button">{{trans('message.create')}}</a>
            @endif
        </div>
    </div>        
    @if(Auth::user()->can('tags_delete'))
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h5>{{trans('message.warning_delete')}} <b><span id="tagId"></span></b> ?</h5>
                </div>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('#deleteModal').on('show.bs.modal', function (event) {
                ...
                var tag_id = link.data('id');
                                
                var modal = $(this);
                modal.find('#deleteForm').attr('action', '{{url('tags')}}/'+tag_id);
                modal.find('#tagId').text(tag_id);
            });
            @endif
