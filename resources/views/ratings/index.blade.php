...
@section('main-content')
    ...
    <div class="row">
        <div class="col-md-12">
                        ...
                        <table id="dataTable" class="table table-hover table-condensed">
                            <thead>
                                <tr>
                                    <th width="3%">ID</th>
                                    <th>Name</th>
                                    @if(Auth::user()->can('ratings_update'))
                                    <th width="5%">{{trans('message.edit')}}</th>
                                    @endif
                                    @if(Auth::user()->can('ratings_delete'))
                            ...
                            @foreach($ratings as $rating)
                                <tr>
                                    ...
                                    @if(Auth::user()->can('ratings_update'))
                                    <td>
                                        <a href="{{url('ratings/'.$rating->id.'/edit')}}">
                                    ...
                                    @if(Auth::user()->can('ratings_delete'))
                                    <td>
                                        <a href="#" data-toggle="modal" data-target="#deleteModal" data-id="{{$rating->id}}">
            ...
            @if(Auth::user()->can('ratings_create'))
            <a class="btn btn-primary" href="{{url('/ratings/create')}}" role="button">{{trans('message.create')}}</a>
            @endif
        </div>
    </div>        
    @if(Auth::user()->can('ratings_delete'))
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h5>{{trans('message.warning_delete')}} <b><span id="ratingId"></span></b> ?</h5>
                </div>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('#deleteModal').on('show.bs.modal', function (event) {
                ...
                var rating_id = link.data('id');
                                
                var modal = $(this);
                modal.find('#deleteForm').attr('action', '{{url('ratings')}}/'+rating_id);
                modal.find('#ratingId').text(rating_id);
            });
            @endif
